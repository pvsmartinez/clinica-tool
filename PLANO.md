...
| Onboarding wizard   | `/onboarding`       | ❌     | Wizard linear de 5 passos (ver abaixo)     |

### Wizard de onboarding — detalhe

Exibido **uma única vez** quando `clinics.onboarding_completed = false`.
Após concluir, redireciona para `/agenda` e não aparece mais.
Os mesmos componentes e configurações são acessíveis depois em `/configuracoes`.

| Passo            | Componente                  | Conteúdo                                                                                                        |
| ---------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------- |
| 1 · Bem-vindo    | `DadosTab` (reuso)          | Confirmar/editar nome da clínica, CNPJ, telefone, e-mail, endereço. Placeholder para dados de cobrança futuros. |
| 2 · Equipe       | `EntityFieldsPanel` (reuso) | Quais campos de profissionais ativar + campos extras personalizados (ex: "Registro CREMERS").                   |
| 3 · Pacientes    | `EntityFieldsPanel` (reuso) | Quais campos de pacientes ativar + campos extras (ex: "Convênio", "Alergias", "Nº prontuário").                 |
| 4 · Agendamentos | `AgendaTab` (reuso)         | Duração do slot + dias/horários de funcionamento.                                                               |
| 5 · Pronto!      | Novo                        | Card de boas-vindas + botão "Abrir minha agenda". Salva `onboarding_completed = true`.                          |

> **Routing guard:** `App.tsx` verifica `clinic.onboardingCompleted` após login.
> Se `false` → `/onboarding`. Se `true` → `/dashboard`.
> A rota `/onboarding` redireciona para `/dashboard` se já foi concluído.
...
| Tela                  | Rota                                   | Status | Descrição                                                              |
| --------------------- | -------------------------------------- | ------ | ---------------------------------------------------------------------- |
| Dados da clínica      | `/configuracoes` (aba Dados)           | ✅     | Nome, CNPJ, endereço, telefone                                         |
| Agenda                | `/configuracoes` (aba Agenda)          | ✅     | Slot duration, working hours por dia da semana                         |
| Campos personalizados | `/configuracoes` (aba Campos)          | ✅     | Toggle de campos padrão + campos extras para pacientes e profissionais |
| Disponibilidade       | `/configuracoes` (aba Disponibilidade) | ✅     | Grade semanal por profissional                                         |
| Usuários e acessos    | ❌                                     | ❌     | Convidar staff, definir perfil (admin/atendente/profissional)          |
| Notificações          | ❌                                     | ❌     | Ativar lembretes, templates de mensagem, antecedência                  |
| Plano e assinatura    | ❌                                     | ❌     | Tier atual, uso, upgrade (Stripe)                                      |
...
| Agenda            | 7      | 1                  | 14%     |
| Pacientes         | 5      | 3                  | 60%     |
| Profissionais     | 3      | 1 (modal cadastro) | 33%     |
| Financeiro        | 4      | 0                  | 0%      |
| Configurações     | 7      | 4                  | 57%     |
...
Sprint 2 — Onboarding wizard
  → Rota /onboarding com 5 passos
  → Routing guard: clinic.onboardingCompleted = false → /onboarding
  → Reuso de EntityFieldsPanel, DadosTab, AgendaTab do SettingsPage
  → Step 5: marcar onboarding_completed = true + redirecionar para /agenda

Sprint 3 — Completar cadastro
  → Upload de arquivos do paciente (Supabase Storage)
  → Histórico de alterações de paciente
  → Gestão de convites e usuários da clínica
...
Sprint 6 — Portal do paciente
  → Agendamento online pelo paciente
  → Meu perfil (edição)
  → Histórico completo com documentos

Sprint 7 — Plano e assinatura
  → Cadastro via tela de onboarding (cartão de crédito)
